...
test('the real bright-objects group splits the way the live catalogue says it does', () => {
  ...
  // 2026-08-05: 157 objects, 94 of them spent stages. If the classifier drifts,
  ...
  // 94, not the 93 this asserted before the three name patterns were unified.
  // The object that moved is ATLAS CENTAUR 2: a spent upper stage whose name
  // contains no "R/B" at all, which the magnitude model had always treated as
  // a stage while classification called it a working payload.
  assert.equal(visual.filter(isDerelictByName).length, 94);
  assert.equal(isDerelictByName('ATLAS CENTAUR 2'), true);
...
test('the launcher names the magnitude model knew about are recognised here too', () => {
  // These disagreed. The brightness estimate treated CENTAUR, BREEZE and
  // FREGAT as upper stages; classification did not. Atlas Centaur 2 was
  // therefore predicted as a bright rocket body and drawn as a working
  // payload simultaneously — and it is on this app's own derelict shortlist.
  for (const name of [
    'ATLAS CENTAUR 2',
    'ATLAS CENTAUR R/B',
    'BREEZE-M',
    ...
    assert.equal(isDerelictByName(name), true, `${name} is a spent stage`);
  }
});

test('the widened pattern still does not swallow working spacecraft', () => {
  for (const name of ['ISS (ZARYA)', 'TERRA', 'AQUA', 'STARLINK-1007', 'XRISM', 'CSS (TIANHE)']) {
