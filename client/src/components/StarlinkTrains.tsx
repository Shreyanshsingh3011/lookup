    ...
    <div className="px-4 py-3 flex flex-col gap-2">
      ...
      {next ? (
        <div className="text-xs text-space-200">
          Next pass <span className="font-mono text-space-100">{formatPassTime(next.start.time)}</span>
          , rising in the {next.start.direction} to{' '}
          <span className="font-mono text-space-100">{next.max.altitudeDeg.toFixed(0)}°</span> in the{' '}
          {next.max.direction}
        </div>
      ) : (
